...
namespace Muon
{
	struct Mesh
	{
		bool Create(const wchar_t* name, UINT vtxDataSize, UINT vtxStride, UINT vtxCount, UINT idxDataSize = 0, UINT idxCount = 0, DXGI_FORMAT idxFormat = DXGI_FORMAT_UNKNOWN, AABB aabb = AABB());
		bool Destroy();
